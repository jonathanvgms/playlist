...
public class PlaylistController : ControllerBase
{
    ...
    [HttpGet]
    public ActionResult<List<PlaylistViewModel>> Get([FromHeader] string token) => Ok(_playlistContext.Playlists.ToList().Adapt<List<PlaylistViewModel>>());


    [HttpGet("{name}")]
    public ActionResult Get([FromHeader] string token, string name)
    {
    ...
    [HttpPost]
    public ActionResult Post([FromHeader] string token, PlaylistViewModel playlist)
    {
    ...
    [HttpPut]
    public ActionResult Put([FromHeader] string token, string name, PlaylistViewModel playlist)
    {
        if (!ModelState.IsValid) return BadRequest("Nombre playlist incorrecto");
    ...
    [HttpDelete]
    public ActionResult Delete([FromHeader] string token, string name)
    {
    ...
    public ActionResult GetSongs([FromHeader] string token, string playlistName)
    {
    ...
    public ActionResult GetSong([FromHeader] string token, string playlistName, string songName)
    {
        var playlist = _playlistContext.Playlists.FirstOrDefault(x => x.Name.ToUpper() == playlistName.ToUpper());
        if (playlist == null) return NotFound($"No se encontro playlist con el nombre: {playlistName}");
    ...
    public ActionResult PostSong([FromHeader] string token, string playlistName, SongViewModel song)
    {
    ...
    public ActionResult PutSong([FromHeader] string token, string playlistName, string songTitle, SongViewModel song)
    {
        if (!ModelState.IsValid) return BadRequest("Nombre playlist incorrecto");
        var playlist = _playlistContext.Playlists.FirstOrDefault(x => x.Name == playlistName);
        if (playlist == null) return NotFound($"No se encontro playlist con el nombre: {playlistName}");
    ...
    public ActionResult PutSong([FromHeader] string token, string playlistName, string songTitle)
    {
        var playlist = _playlistContext.Playlists.FirstOrDefault(x => x.Name == playlistName);
        if (playlist == null) return NotFound($"No se encontro playlist con el nombre: {playlistName}");
